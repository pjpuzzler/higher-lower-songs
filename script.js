// Read implicit flow hash (for backward compatibility) and query params (PKCE)
...
// Try to reuse stored access token if still valid
const storedAccess = localStorage.getItem("access_token");
const storedExpiresAt = Number(localStorage.getItem("expires_at") || 0);
const storedRefresh = localStorage.getItem("refresh_token");
const wasSignedIn = localStorage.getItem("signed_in") === "true";

if (storedAccess && Date.now() < storedExpiresAt) {
  _token = storedAccess;
...
} else if (storedRefresh && wasSignedIn) {
  // Try refresh token to obtain a new access token without prompting the user
  (async () => {
    try {
      await refreshAccessToken();
    } catch (e) {
      console.warn("Refresh token failed:", e);
      // fallback to signed-out anonymous mode; do not auto-prompt consent
      localStorage.setItem("signed_in", false);
    }
  })();
} else if (hash.access_token) {
  // Backwards compatibility: implicit grant returned token in URL hash
  _token = hash.access_token;
  signedIn = false; // implicit tokens are short-lived; treat as not fully signed-in
  if (hash.expires_in) {
    const expires = Date.now() + Number(hash.expires_in) * 1000 - 30000;
...
window.onresize = () => {
  if (document.getElementsByTagName("body")[0].className !== "adaptive") return;
...
const load = async () => {
  ...
  const elUserImageContainer = document.getElementById("user_image_container"),
  ...
  if (signedIn) {
    ...
    if (userPlaylists.items.length > 0) {
      document.getElementById("use_user_playlist_label").style.color = null;
      ...
      for (const userPlaylist of userPlaylists.items) {
        try {
          elUserPlaylist.add(new Option(userPlaylist.name, userPlaylist.id));
        } catch {}
      }
    }
    ...
    if (
      !userPlaylists.items.some(
        (userPlaylist) => userPlaylist?.id === params.songs.userPlaylistId
      )
    )
...
if (authCode) {
  // Currently exchanging code for token; exchangeCodeForToken will call waitForLoad()
} else if (!_token) {
  signedIn = false;
  ...
  $.ajax({
    ...
    headers: {
      Authorization:
        "Basic " + window.btoa(CLIENT_ID + ":b70cb856044a46388dbacd8c753dd232"),
    },
...
function updateParams() {
  ...
  document.getElementById("uri_search").value = params[mode].uriSearch?.q ?? "";
  ...
  if (mode === "songs") {
    switch (params[mode].use) {
      ...
      case "top": {
        ...
        document.getElementById("source_text").innerText = `Top (${maxOffset})`;
    ...
    switch (params[mode].use) {
      ...
      case "uri": {
        ...
        try {
          [artistData, artistAlbumData] = await Promise.all([
            getData(`https://api.spotify.com/v1/artists/${artistId}`, false),
            getData(
      ...
      case "search": {
        ...
        document.getElementById("source").href =
          "https://open.spotify.com/search/" + queryString + "/albums";
        document.getElementById(
          "source_text"
    ...
    switch (params[mode].use) {
      case "liked": {
        ...
        for (const artist of followedArtistData.artists.items)
          urlsLeft.push(`https://api.spotify.com/v1/artists/${artist.id}`);
  ...
  return new Promise((resolve, reject) => {
    $.ajax({
      ...
      success: (data) => {
        ...
        if (type === "songs") {
          const trackData =
            data.tracks?.items[0] ?? data.items?.[0].track ?? data.items?.[0];
              ...
              fetchJsonp(
                ...
                .then((deezerData) => {
                  if (deezerData.preview) {
                    trackData.preview_url = deezerData.preview;
                  }
                  resolve(trackData);
                })
                .catch(() => {
              ...
              success: (data) => {
                ...
                if (isrc) {
                  fetchJsonp(
                    ...
                    .then((deezerData) => {
                      if (deezerData.preview) {
                        data.preview_url = deezerData.preview;
                      }
                      resolve(data);
                    })
                    .catch(() => {
          ...
          const albumData =
            data.albums?.items[0] ?? data.items?.[0].album ?? data.items?.[0];
          ...
          const artistData =
            data.artists?.items[0] ?? data.items?.[0] ?? data.tracks ?? data;
...
function updateMarquees(sideNum) {
  ...
  marqueeTimesoutIds[sideNum + 3] = setTimeout(() => {
    const elTrackInfo = document.getElementById(
      ...
      marginLeftOffset =
        (window.matchMedia("(orientation:portrait)").matches ? 0.008 : 0.015) *
        document.documentElement.scrollHeight,
      explicitOffset =
        (window.matchMedia("(orientation:portrait)").matches ? 0.0025 : 0.005) *
        document.documentElement.scrollHeight;
    ...
    if (trackTitleWidth > trackInfoWidth - marginLeftOffset - gradientWidth) {
      const titleMarqueeProperty = `--marquee_title_${sideNum}_distance`,
        titleMarqueeDistance =
          trackTitleWidth - (trackInfoWidth - marginLeftOffset - gradientWidth);
  ...
  const albumData = sideNum === 1 ? albumData1 : albumData2,
    ...
    trackData =
      mode === "songs"
        ...
        : artistData.preview_track,
    noAudio =
      !trackData?.preview_url || (params.muteExplicit && trackData.explicit),
    explicit = trackData && trackData.explicit,
    ...
    if (!reveal && sideNum === 1 && useRight) {
      ...
      Vibrant.from(albumArtUrl)
        ...
        .then((palette) => {
          ...
          const leftTitleMarqueeGradient = getTitleMarqueeLinearGradient(
              hsl,
              "right"
            ),
            rightTitleMarqueeGradient = getTitleMarqueeLinearGradient(
              hsl,
              "left"
            ),
            leftArtistMarqueeGradient = getArtistMarqueeLinearGradient(
              hsl,
              "right"
            ),
            rightArtistMarqueeGradient = getArtistMarqueeLinearGradient(
              hsl,
              "left"
            );

          elTrackLeftGradient.style.background = leftTitleMarqueeGradient;
          elTrackRightGradient.style.background = rightTitleMarqueeGradient;
          elArtistLeftGradient.style.background = leftArtistMarqueeGradient;
          elArtistRightGradient.style.background = rightArtistMarqueeGradient;
        });
  ...
  if (reveal || !params.soundOnly) {
    if (mode === "artists") {
      ...
      for (
        ...
        const artist = (mode === "songs" ? trackData : albumData).artists[i],
      ...
      complete: () => {
        if (type !== 2) {
          ...
          if (streak % STREAK_LENGTH === 0 && streak > 0) {
            ...
            document.getElementById("skip_button").style.display = "flex";
...
function checkGuess(higher) {
  ...
  const skip = higher === null,
    correct = higher ? popularity2 >= popularity1 : popularity2 <= popularity1;
    ...
    checkPromises.push(
      new Promise(async (resolve, reject) => {
        try {
          ...
          else if (mode === "albums") albumDataTmp = await getRandomAlbumData();
          else if (mode === "artists") {
  ...
  Promise.allSettled(checkPromises).then((results) => {
    ...
    if (correct) {
      ...
      if (!params.zen && score > (highScores[paramKey] ?? 0)) {
        ...
        if (signedIn) window.storeHighScore(userData.id, paramKey, score);
        else localStorage.setItem("high_scores", JSON.stringify(highScores));
...
async function getRandomTrackData() {
  ...
  while (true) {
    ...
    invalidForSoundOnly =
      !trackData.preview_url || (trackData.explicit && params.muteExplicit);
...
async function getRandomArtistData() {
  ...
  artistData.preview_track.videoSrc = await getMovie(artistData.preview_track);
...
function updatePlayValidity(forceDisable = false) {
  ...
  if (
    !forceDisable &&
    ((params[mode].use === "user_playlist" && params[mode].userPlaylistId) ||
      params[mode].use === "liked" ||
      params[mode].use === "top" ||
      (params[mode].use === "search" &&
        (mode === "songs"
          ? yearCode >= 1 || (params[mode].query.genre && yearCode !== -1)
          : mode === "albums"
          ? yearCode >= 1
          : yearCode === 1 ||
            (params[mode].query.genre && yearCode !== -1 && yearCode !== 2))) ||
      (params[mode].use === "uri" &&
        (mode === "songs"
...
async function exchangeCodeForToken(code) {
  ...
  _token = data.access_token;
  signedIn = true;
  // Persist tokens and expiry so refresh/refresh-less reloads don't re-prompt
...
async function refreshAccessToken() {
  ...
  if (!resp.ok) {
    const txt = await resp.text();
    throw new Error(`Refresh token endpoint error: ${resp.status} ${txt}`);
  }
...
async function signIn(showDialog = false) {
